...
**v0.1.0-alpha.4** — all twenty steps, all thirty-four careers, and every
species a setting data file declares. This release is what one afternoon of
playtesting alpha.3 against a transcribed setting file turned up: a homeworld
can be chosen rather than only rolled, which is what pp. 39-40 say and what
reaches the fourteen worlds of p. 55 that nothing could; and three findings
that put a wrong character on the sheet without saying so — a baseline human
choosing the career the book reserves for the enslaved, `(Any)` on skills that
have none, and a subsector throw into a chart gap landing most of a batch on
one subsector.
...
The schema is versioned and so is the policy, and a record carries both. A
record still replays under a later version where the engine has not changed
what a seed produces, and says where it diverged where it has — alpha.3's
records do not replay under this release, because `(Any)` no longer lands on a
skill that has none. That is a fix changing an answer rather than a format
changing shape, so the message names the value that moved rather than refusing
the file.
